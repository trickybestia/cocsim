...
class MainWindow:
    ...
    def on_left_click(self, event):
        ...
        if self.selection_start_pos is None:
            ...
            def on_building_selected(building: Type[Building]):
                if building is Wall:
                    for x in range(selection_size[0]):
                        for y in range(selection_size[1]):
                            self._add_building(
                                building, building_x + x, building_y + y
                            )
                else:
                    self._add_building(building, building_x, building_y)
    ...
    def _add_building(self, building: Type[Building], tile_x: int, tile_y: int):
